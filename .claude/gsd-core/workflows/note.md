...
- **Global scope**: `/opt/norish-src/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/opt/norish-src/.claude/notes/`)
...
2. Glob `/opt/norish-src/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/opt/norish-src/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/opt/norish-src/.claude/notes/` — works in any directory
