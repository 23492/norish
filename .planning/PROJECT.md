...
| **Sole worker = Antigravity (Gemini 3.5 Flash, aggressive `--think`) on the personal Plus sub**; DeepSeek disabled for now | Google ships `agy` as a first-party CLI included in the subscription (sanctioned, no extra billing) — unlike Anthropic's banned subscription-via-proxy. Plus is rate-capped, so `tooling/cross-ai/run-or-defer.sh` + a cron drainer make it quota-aware: try → on `RESOURCE_EXHAUSTED` record a cooldown, keep the task queued, let cron resume when quota returns. DeepSeek executor stays in place but disabled (`NORISH_CROSS_AI_ALLOW_DEEPSEEK=1` to re-enable). | Done (2026-06-22) |
