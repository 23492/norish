...
| **Default worker = Antigravity (Gemini 3.5 Flash, aggressive `--think`) on the personal Plus sub**; DeepSeek as fallback | Google ships `agy` as a first-party CLI included in the subscription (sanctioned, no extra billing) — unlike Anthropic's banned subscription-via-proxy. Plus is rate-capped, so `tooling/cross-ai/run-or-defer.sh` + a cron drainer make it quota-aware: try → on `RESOURCE_EXHAUSTED` record a cooldown from the parsed reset, keep the task queued, and let cron resume when quota returns. DeepSeek (paid key, no quota wall) stays available for when the weekly cap is spent. | Done (2026-06-22) |
