...
- [ ] **Phase 27: Cooklang migration (MAJOR)** - Migrate recipe step/ingredient representation to Cooklang, delivering the long-wanted **in-step ingredient quantities** and **multi-timer cooking mode**. **UNBLOCKED 2026-07-24 (Kiran): fork-independent, NO #470 gating** — the fork has progressed past the original and makes its own design calls, so the serializer lives **fork-local** rather than being contributed upstream. Parser `@cooklang/cooklang` (MIT, WASM — NOT the archived `cooklang-ts`). Resolved design (D-1..D-8, sequencing, waves) in **`.planning/phases/27-cooklang/27-DECISIONS.md`**, de-risked by a committed spike (working `structuredToCooklang` serializer + tests validated against the REAL WASM parser, improved extraction prompt, and an AI-relinking experiment — `27-EXPERIMENT.md`). Shape: additive nullable `cook_source` (`0041_*`) alongside the still-authoritative structured tables (D-1); AI emits structured JSON with per-step ingredient refs → pure serializer emits `.cook` (D-3); AI-backfill behind a **confidence gate + `SmartInstruction` fallback** (D-6/D-7). (COOK-01)
...
**Status**: **UNBLOCKED 2026-07-24 (Kiran)** — fork-independent, **no #470 gating**; the fork makes its own design calls and keeps the serializer fork-local. Design resolved (D-1..D-8, sequencing, wave plan) in `.planning/phases/27-cooklang/27-DECISIONS.md`; de-risked by a committed spike (`27-cooklang/spike/` serializer + tests against the REAL `@cooklang/cooklang@0.18.7` WASM parser, `27-EXTRACTION-PROMPT.md`, and the AI-relinking `27-EXPERIMENT.md`).
...
**Success Criteria**:
  ...
  2. A fork-local structured→`.cook` serializer (`structuredToCooklang` in `@norish/shared`, D-4) lets the AI/JSON-LD importer emit Cooklang; prototype built + tested in the spike.
  3. Dual metric/US stays in the structured tables; the `.cook` carries ONE unit system (D-2); canonical unit IDs round-trip as the `%unit` literal (D-8, confirmed).
  4. A cooking mode drives multiple named timers concurrently from the parsed steps.
  5. Additive/reversible: `0041_*` adds a nullable `cook_source` while `steps`/`recipe_ingredients` stay authoritative (D-1); existing recipes are AI-relinked behind a confidence gate with `SmartInstruction` fallback (D-6/D-7).
**Waves** (see 27-DECISIONS.md): W1 read-path parser+renderer · W2 serializer + `0041` write-path · W3 AI backfill + improved prompt + confidence gate · W4 multi-timer (secondary).
**Plans**: TBD — plannable now (design resolved).
