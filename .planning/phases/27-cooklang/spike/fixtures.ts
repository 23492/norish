// SPIKE — Phase 27 (COOK-01). Five realistic norish recipes.
// -----------------------------------------------------------------------------
// No recipe fixtures/seed data exist in the repo (only seed-config.ts), so these
// are hand-constructed to match REAL norish shape: a flat `steps` list + a
// SEPARATE structured `ingredients` list with NO linkage (exactly what the DB
// holds today). Each recipe also carries the AFTER form: `linkedSteps`, where I
// (the model, playing the AI-backfill pass) mapped each ingredient to the step(s)
// that use it. `expected` records, per step, the amounts that MUST survive the
// structured -> .cook -> parse round-trip. Ambiguities/failures are annotated in
// 27-EXPERIMENT.md.
// -----------------------------------------------------------------------------

import type { SpikeRecipe } from "./structured-to-cooklang";
...
export type Fixture = {
  ...
  // AFTER: my AI re-linking, as serializer input.
  recipe: SpikeRecipe;
  // per-step expected inline ingredients (name -> {amount, unit}) after round-trip
  expected: { stepText: string; ingredients: { name: string; amount: number | null; unit: string | null }[] }[];
};
