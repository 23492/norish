...
The system SHALL persist an operation receipt for each replay-safe mutation execution keyed by mutation family, actor scope, and `operationId`.
...
The system SHALL reject reuse of the same `operationId` when the later request does not match the stored receipt's mutation family, actor scope, and semantic payload fingerprint.

#### Scenario: Same operationId reused for a different payload
- **WHEN** a request reuses an `operationId` that is already bound to a different semantic command
